/** Native comparison starts only where both files have a corresponding frame. */
export function sharedVideoStart(referenceTime:number,origins:number[],bpm:number[]){
 return Math.max(referenceTime,0,origins[0]-origins[1]*bpm[1]/bpm[0]);
}
