...
export function validateConfig(input:unknown):PracticeConfig{
 ...
 for(const [key,v] of Object.entries(value)){const min=key==='rate'?.25:40,max=key==='rate'?2:300;if(typeof v!=='number'||!Number.isFinite(v)||v<min||v>max)throw new Error(`Invalid ${key}: expected ${min}–${max}`);}
