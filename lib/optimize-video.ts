export async function optimizeVideo(file:File,signal:AbortSignal,onProgress:(n:number)=>void):Promise<Blob>{
 ...
 try{
  ...
  try{if(Math.abs(await check.computeDuration()-duration)>.15)throw new Error('軽量化で動画の長さが変わったため適用しませんでした。元の動画を使います。');}finally{check.dispose();}
  return blob;
