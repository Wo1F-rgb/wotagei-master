...
export async function analyzeRhythmFile(file:Blob,signal:AbortSignal,onStage:(message:string)=>void):Promise<RhythmAnalysisResult>{
 if(file.size>160*1024*1024)throw new Error('160MBを超える動画は、音源ファイルを選ぶか短く切り出してください。');
 signal.throwIfAborted();onStage('動画・音源の音を読み込み中…');
 const context=new OfflineAudioContext(1,1,22050);let buffer:AudioBuffer;
 try{buffer=await context.decodeAudioData(await file.arrayBuffer());}catch{signal.throwIfAborted();throw new Error('音を読み出せませんでした。MP3・M4A・WAVの音源でも解析できます。');}
 signal.throwIfAborted();if(buffer.duration<8||buffer.duration>600)throw new Error('8秒〜10分の音源で解析してください。');
