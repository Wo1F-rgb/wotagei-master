...
import { Activity, ArrowLeft, Camera, Check, ChevronLeft, ChevronRight, Columns2, Share2, ExternalLink, FlipHorizontal2, Layers2, Maximize, Minimize, Pause, Play, Repeat2, RotateCcw, Settings2, SkipBack, SkipForward, SlidersHorizontal, Square, SwitchCamera, Upload, X } from 'lucide-react';
...
import {RecordingSaveDialog} from '@/components/recording-save-dialog';
import {recordingSoundLabel} from '@/lib/recording-sound';
import {cameraFrame,cameraFormatLabel,cameraFacingLabel,cameraZoomClip,type CameraFacing} from '@/lib/camera-recording';
...
export default function Home(){
 const s=useStudio();const tempoConfig=useRef<TempoConfigHandle>(null);const [recordingDialog,setRecordingDialog]=useState<'sound'|'screen'|null>(null);
 const fullscreen=useStudioFullscreen(s.setNotice);
 useEffect(()=>{if(s.recordingSave.open&&fullscreen.expanded)void fullscreen.exit();},[s.recordingSave.open,fullscreen.expanded,fullscreen.exit]);
 useEffect(()=>{setRecordingDialog(null);},[s.sources[0]?.url,s.camera]);
 function recordingAction(){if(s.recording||s.recordingBusy){void s.toggleRecording();return;}if(s.sources[0]?.youtube){s.clearRecordingError();setRecordingDialog('sound');}else void s.toggleRecording();}
 ...
 const firstBeatOriginal=useRef<number[]|null>(null);
 useEffect(()=>{firstBeatOriginal.current=null;setPanel(p=>p==='firstBeat'?null:p);},[s.sources[0]?.url,s.sources[1]?.url]);
 ...
 function toggleFullscreen(){if(!fullscreen.expanded){discardFirstBeat();if(config!==null)closeConfig();setPanel(null);setYoutubeControls(false);}void fullscreen.toggle();}
 function openScreenRecordingView(){
  // iOS owns screen recording. Prepare a paused viewer, never a MediaRecorder
  // or a pretend REC state; the user starts recording in Control Center.
  s.pause();s.setNotice('');discardFirstBeat();setConfig(null);setPanel(null);setYoutubeControls(false);setRecordingDialog(null);
  if(!fullscreen.expanded)void fullscreen.toggle();
 }
 ...
 function openMotion(){discardFirstBeat();s.pause();setMotionPending(false);if(missingBpm!==null)openConfig(missingBpm,'tempo');else if(!motionCurve?.frames.length&&!motionHint)startSequence(true);else{setMode('overlay');setPanel('alignment');}}
 ...
 // Trial seeks and preview origins are local to the first-beat editor. Every
 // exit restores the saved grid and aligns the follower to the audible master.
 function discardFirstBeat(){const original=firstBeatOriginal.current;if(!original)return;firstBeatOriginal.current=null;s.setOrigins([...original]);s.finishTimingEdit();}
 function loadPracticeFile(index:number,file:File){discardFirstBeat();s.loadFile(index,file);}
 function openFirstBeat(){firstBeatOriginal.current=[...s.origins];s.pause();s.setNotice('');s.seekSolo(0,s.origins[0]);s.seekSolo(1,s.origins[1]);setConfig(null);setPanel('firstBeat');}
 function openConfig(index:number,tab?:ConfigStep){discardFirstBeat();poseRequest.current++;setPoseBusy(false);setPoses(null);setPanel(null);s.pause();s.setNotice('');setStep(tab||(s.sources[index]&&!(index===1&&s.camera)?'tempo':'source'));setConfig(index);}
 function removeVideo(index:number){discardFirstBeat();s.removeVideo(index);setConfig(null);setPanel(null);}
 ...
 const fileInput=(i:number,label:string)=><label className="button file-button"><Upload size={17}/>{label}<input type="file" accept="video/*,.mov,.mp4,.webm" disabled={s.recording||s.recordingBusy} aria-label={`${i===0?'お手本':'自分'}の動画を選ぶ`} onChange={e=>{const f=e.target.files?.[0];if(f)loadPracticeFile(i,f);e.target.value='';}}/></label>;
 const recentPicker=(i:number)=><RecentMediaPicker index={i} loadFile={file=>loadPracticeFile(i,file)} loadLink={saveLink} disabled={s.recording||s.recordingBusy} pause={s.pause}/>;
 ...
 return <main data-fullscreen={fullscreen.mode} className={'studio '+(immersive?'studio-immersive ':'')+(fullscreen.expanded?'studio-expanded ':'')+(config!==null?'editing ':'')+(choosing?'choosing ':'')+(isYoutube?'has-youtube ':'')+(panel==='alignment'||panel==='firstBeat'?'aligning':'')+(panel==='firstBeat'?' first-beat-timing':'')}>
  ...
  <div ref={decksRef} style={{'--packed-columns':packed.columns,'--packed-rows':packed.rows} as CSSProperties} className={'decks '+(overlay?'overlaid '+(s.alignmentMaster===1?'alignment-master-self ':''):'')+(overlay&&isYoutube?'youtube-overlaid ':'')+(youtubeControls?'youtube-controls-visible ':'')+(config!==null?'configuring':'')}>
   {[0,1].map(i=>{
    ...
    const selectedLens=live?s.cameraDevices.find(d=>d.id===s.cameraLens.deviceId):undefined;
    const lensLabel=selectedLens?({ultrawide:'超広角',wide:'標準',telephoto:'望遠',other:selectedLens.label})[selectedLens.kind]:'';
    const digitalZoom=live?s.cameraLens.digital:1;
    const cameraPoints=livePoints.points.map(p=>{if(digitalZoom===1)return p;const x=(p.x-.5)*digitalZoom+.5,y=(p.y-.5)*digitalZoom+.5;return {...p,x,y,visibility:x<0||x>1||y<0||y>1?0:p.visibility};});
    const boneBase=displayPose(cameraPoints.map(p=>projectPoint(sceneMatrix(boneScene),p)),boneSize,live&&!overlay?{width:frameRect.width,height:frameRect.height}:boneStage,!calibrating&&s.mirrors[i]);
    ...
    const mediaStyle={...cameraPreviewStyle,filter,clipPath:live?cameraZoomClip(boneSize.width,boneSize.height,overlay?viewSize:frameRect,digitalZoom):undefined,transform:baseTransform+(live?(digitalZoom>1?` scale(${digitalZoom})`:''):' '+sceneCss(calibrating?(sceneScope[i]===backgroundKeys[i]?scenePreview[i]:null):currentScenes[i],viewSize)),opacity:overlay&&i===s.alignmentTarget&&!(yt&&youtubeControls)?s.alignment.opacity:1};
    return <section className={`deck deck-${i} ${selected?'selected':''} ${drag===i?'dragging':''}`} key={i} onDragOver={e=>{e.preventDefault();setDrag(i);}} onDragLeave={()=>setDrag(null)} onDrop={e=>{e.preventDefault();setDrag(null);if(s.recording)return;const f=e.dataTransfer.files[0];if(f)loadPracticeFile(i,f);}}>
     <header className="deck-header"><span className="deck-dot"/><strong>{i===0?'お手本':'自分'}</strong>{yt&&overlay&&<button className="button mini youtube-overlay-toggle" disabled={!s.sources[1]&&!s.camera} aria-pressed={youtubeControls} onClick={()=>setYoutubeControls(v=>!v)}>{youtubeControls?'重ね表示に戻る':'YouTubeを操作'}</button>}{source?.twitter?<a className="filename" href={source.twitter.url} target="_blank" rel="noopener noreferrer" title="Xの元投稿を開く">{source.name} ↗</a>:<span className="filename" title={live?selectedLens?.label:source?.name}>{live?cameraFacingLabel[s.cameraFacing]+(lensLabel?' · '+lensLabel:''):source?.name||'動画未選択'}</span>}{source?.twitter&&<TwitterVideoSwitch source={source.twitter} loading={s.linkLoading} disabled={s.recording||s.optimizing||poseBusy||sequence!==null} select={value=>void changeTwitterVideo(value)} cancel={()=>{s.cancelLinkLoad();s.setNotice('');}}/>}<span className="deck-status">{live?'LIVE · '+cameraFormatLabel[s.cameraFormat]:i===1&&s.optimized?'軽量版':s.bpmKinds[i]==='unset'?'BPM 未設定':`${Number(s.bpm[i].toFixed(3))} BPM`}</span></header>
     <div className="deck-body"><div ref={i===1?selfStage:referenceStage} style={{mixBlendMode:overlay&&appearance.tint&&cutReady?'screen':undefined}} className={'video-stage '+(selected&&available&&!yt?'with-preview ':'')+(yt?'youtube-stage':'')+(cutReady&&!overlay?' cutout-preview '+backgrounds[i].preview:'')}>
      ...
      {yt&&source?.youtube?<YouTubeReference key={source.key+":"+source.instance} link={source.youtube} style={overlay&&!youtubeControls?{transform:alignmentCss(s.alignments[0],stageSize),opacity:mediaStyle.opacity}:undefined} ready={s.attachYoutube} state={s.youtubeState} rate={s.youtubeRate} error={s.youtubeError} metadata={s.youtubeMetadata}/>:<video ref={i===0?s.reference:s.self} src={source?.url} playsInline muted={live||i!==(s.soloPlaying??s.soundSource)} preload="auto" style={{...mediaStyle,opacity:cutReady?0:mediaStyle.opacity}} onResize={e=>readVideoSize(i,e.currentTarget)} onLoadedMetadata={e=>{readVideoSize(i,e.currentTarget);if(!live)s.loaded(i);}} onEnded={()=>s.mediaEnded(i)} onWaiting={()=>s.mediaWaiting(i)} onSeeking={()=>s.interruptTap(i)} onSeeked={s.mediaClockChanged} onTimeUpdate={s.mediaClockChanged} onPause={e=>{if(e.currentTarget.paused)s.interruptTap(i);s.mediaClockChanged();}} onPlaying={i===0?s.mediaPlaying:undefined} onError={()=>{if(source&&!live)s.mediaError(i);}} aria-label={i===0?'お手本の映像':'自分の映像'}/>}
      ...
      {live&&<CameraZoomControls lens={s.cameraLens} devices={s.cameraDevices} facing={s.cameraFacing} busy={s.cameraBusy} locked={s.recording||s.recordingBusy} change={value=>void s.setCameraZoom(value)}/>}
      ...
      {i===0&&!yt&&s.buffering&&<span className="stage-badge">読み込み中</span>}{live&&s.recording&&<span className="stage-badge rec" role="status">● REC · {recordingSoundLabel[s.recordingSound]}</span>}{live&&!s.recording&&s.recordingDownload&&<button className="stage-badge recording-save" onClick={s.recordingSave.reopen}><Share2 size={14}/>録画を保存</button>}
     ...
     {selected&&<Tabs className="video-config" data-step={step} value={step} onValueChange={v=>changeStep(String(v))}><TabsList aria-label="動画設定の項目"><TabsTrigger value="tempo">拍・BPM</TabsTrigger><TabsTrigger value="origin">拍の位置</TabsTrigger><TabsTrigger value="display">表示</TabsTrigger><TabsTrigger value="camera">画角</TabsTrigger><TabsTrigger value="source">動画</TabsTrigger></TabsList>{step==='origin'&&<BeatPreview time={t} origin={s.origins[i]} bpm={s.bpm[i]} playing={s.beatPreview===i} disabled={!available||s.bpmKinds[i]==='unset'} toggle={()=>s.beatPreview===i?s.pause():void s.previewBeats(i)}/>}
      ...
      <TabsContent value="source" className="config-content source-content">{sourceChoices(i,source&&!live?'別の動画を選ぶ':'選ぶ')}
       {i===1&&s.camera&&s.cameraDevices.length>0&&<label className="numeric camera-lens-settings"><span>使用するレンズ</span><select aria-label="使用するカメラのレンズ" value={s.cameraLens.deviceId} disabled={s.cameraBusy||s.recording||s.recordingBusy} onChange={e=>void s.selectCameraDevice(e.target.value)}>{!s.cameraDevices.some(d=>d.id===s.cameraLens.deviceId)&&<option value={s.cameraLens.deviceId}>現在のカメラ</option>}{s.cameraDevices.map(d=><option value={d.id} key={d.id}>{d.label}</option>)}</select></label>}
  ...
  <RecordingSaveDialog save={s.recordingSave}/>
  <RecordingAudioDialog open={recordingDialog!==null} screenGuide={recordingDialog==='screen'} screenView={openScreenRecordingView} busy={s.recordingBusy} error={s.recordingError} start={s.toggleRecording} close={()=>{s.cancelRecordingStart();setRecordingDialog(null);}}/>
