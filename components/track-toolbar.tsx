...
export function TrackToolbar({master,rate,changeMaster,changeRate,canSelf,disabled,nudgeDisabled,nudgeStep,nudge,scope}:TrackControls){
 ...
 return <div className="track-toolbar compact-toolbar" data-target={master===0?'self':'reference'}>
  ...
  {([-1,1] as const).map(direction=><button key={direction} className="track-nudge" data-fine={stepLabel.length>5} aria-label={`${target}の拍を${stepLabel}秒${direction===-1?'早める':'遅らせる'}`} title={`${target}の曲に対する拍を${stepLabel}秒${direction===-1?'早く':'遅く'}して保存。次の同期再生で合わせます。`} disabled={nudgeDisabled} data-feedback={flash&&feedback?.direction===direction?(feedback.reason?'blocked':'applied'):undefined} onClick={()=>move(direction)}><span key={feedback?.direction===direction?feedback.count:0}>{direction===-1?`←−${stepLabel}`:`+${stepLabel}→`}</span></button>)}
  ...
  <output className="track-nudge-feedback" data-visible={flash} data-blocked={!!feedback?.reason} aria-live="polite" aria-atomic="true">{feedback?`${feedback.reason||`${target}の拍 ${total}秒 保存済み`} · ${feedback.count}回`:''}</output>
 </div>;
}
