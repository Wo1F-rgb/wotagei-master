...
export type TrackControls={master:0|1;rate:number;changeMaster:(n:0|1)=>void;changeRate:(n:number)=>void;canSelf:boolean;disabled:boolean;nudgeDisabled:boolean;nudge:(direction:-1|1)=>NudgeResult;scope:string};
export function TrackToolbar({master,rate,changeMaster,changeRate,canSelf,disabled,nudgeDisabled,nudge,scope}:TrackControls){
 ...
 const total=feedback?`${feedback.total>0?'+':feedback.total<0?'−':'±'}${Math.abs(feedback.total).toFixed(3)}`:'';
