...
export function AppearanceControls({value,change,disabled=false,analysis,live=false}:{value:VideoAppearance;change:(value:VideoAppearance)=>void;disabled?:boolean;analysis?:ReactNode;live?:boolean}){
 ...
 {value.skeleton&&!disabled&&analysis}<p className="config-hint">{disabled?'YouTubeの骨格・人物切り抜きは利用できません。':(live?'カメラの骨格は撮影中に解析します。':'動画の骨格は先に全体を解析し、再生フレームに合わせて表示します。解析結果はこのページを開いている間は再利用します。')+' 見えない関節は線を消します。色分けは下の「背景」で切り抜きをONにすると反映されます。'} 明るさ・色分けは表示用で、元動画や録画は変わりません。</p></div>;
}
