...
export function MotionControls({target,curve,bothMoving,strength,changeStrength,start,disabled,hint}:{target:0|1;curve:MotionCurve|null;bothMoving:boolean;strength:number;changeStrength:(v:number)=>void;start:()=>void;disabled:boolean;hint:string}){
 ...
 return <div className="motion-controls" aria-label="手持ち動画の追従">
  ...
  <p>{hint|| (curve?.frames.length?`追従準備済み · 有効な場面 ${Math.round(curve.accepted/curve.total*100)}%。再生すると位置・倍率が追従します。`:'BPM・拍と基本の位置を合わせてから解析してください。再生中に骨格解析は行いません。')}</p>
  <p>{bothMoving?'両方が三脚OFFなので、位置の主役を基準に相手が追従します。':'三脚ON側を固定し、OFF側だけを補正します。'} 傾き・遠近は固定。検出できない場面は補正を弱めます。</p>
