...
export function RecentMediaPicker({index,loadFile,loadLink,disabled,pause}:Props){
 ...
 <div className="recent-items">{visible.map(item=><div className="recent-item" key={item.id}><button disabled={busy} onClick={()=>void choose(item)}><strong>{item.name}</strong><small>{item.kind==='file'?`${(item.size/1024/1024).toFixed(1)} MB`:item.kind==='youtube'?'YouTube':'Twitter / X'} · {new Date(item.usedAt).toLocaleDateString('ja-JP')}</small></button><button className="icon-button" disabled={busy} aria-label={`${item.name}を履歴から削除`} onClick={()=>void remove(item.id)}><Trash2 size={17}/></button></div>)}{!visible.length&&<p>履歴はまだありません。動画ファイルやYouTube・Xのリンクを選ぶと、ここから再び使えます。</p>}</div>
