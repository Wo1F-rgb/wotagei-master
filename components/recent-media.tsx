...
export function RecentMediaPicker({index,loadFile,loadLink,disabled,pause}:Props){
 ...
 const visible=items.filter(item=>index===0||item.kind==='file');
 return <><button className="button" disabled={disabled} onClick={()=>{pause();setError('');setMessage('');setClearConfirm(false);setOpen(true);}}><History size={17}/>最近使ったもの</button>
 ...
 <p className="config-hint">動画は10本・合計1GBまで（1本500MBまで）。ブラウザのデータ削除や空き容量不足で消える場合があります。元動画は写真アプリなどにも残してください。別端末とは同期しません。</p>
