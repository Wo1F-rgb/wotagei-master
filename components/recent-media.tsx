...
export function RecentMediaPicker({index,loadFile,loadLink,disabled,pause}:Props){
 ...
 return <><button className="button" aria-label={`${index===0?'お手本':'自分'}の履歴を開く`} disabled={disabled} onClick={()=>{pause();setError('');setMessage('');setClearConfirm(false);setOpen(true);}}><History size={17}/>履歴</button>
  <Dialog open={open} onOpenChange={v=>{setOpen(v);if(!v){request.current++;setBusy(false);}}}><DialogContent className="practice-dialog history-dialog"><DialogTitle>{index===0?'お手本':'自分'}の履歴</DialogTitle><DialogDescription>この端末だけに保存。動画はアップロードしません。</DialogDescription>
  <label className="setting-row"><span>履歴に保存</span><Switch checked={remember} onCheckedChange={v=>{try{setHistoryEnabled(v);setRemember(v);}catch(e){setError(historyError(e));}}}/></label>
  <div className="recent-items">{visible.map(item=><div className="recent-item" key={item.id}><button disabled={busy} onClick={()=>void choose(item)}><strong>{item.name}</strong><small>{item.kind==='file'?`${(item.size/1024/1024).toFixed(1)} MB`:item.kind==='youtube'?'YouTube':'Twitter / X'} · {new Date(item.usedAt).toLocaleDateString('ja-JP')}</small></button><button className="icon-button" disabled={busy} aria-label={`${item.name}を履歴から削除`} onClick={()=>void remove(item.id)}><Trash2 size={17}/></button></div>)}{!visible.length&&<p>履歴はありません。</p>}</div>
 {error&&<p className="warning" role="alert">{error}</p>}{message&&<p role="status">{message}</p>}
  <p className="config-hint">最大10件。動画は合計1GB・1本500MBまで。端末のデータ削除で消える場合があります。</p>
 <div className="source-actions"><button className="button" onClick={()=>void(async()=>{try{const granted=await navigator.storage?.persist?.();setMessage(granted?'このブラウザで保存領域の保護が有効です。':'保存領域の保護は許可されませんでした。通常の保存は使えます。');}catch{setMessage('このブラウザでは保存領域を保護できません。');}})()}>保存領域を保護</button>{items.length>0&&!clearConfirm&&<button className="button" onClick={()=>setClearConfirm(true)}>履歴をすべて削除</button>}{clearConfirm&&<><button className="button" disabled={busy} onClick={()=>void remove()}>全履歴を削除する</button><button className="button" onClick={()=>setClearConfirm(false)}>やめる</button></>}</div>
  <p className="config-hint">削除対象はアプリ内コピーと解析履歴。元動画は削除しません。OFFでも既存履歴は残ります。</p>
