...
export function BeatTap({recording,count,grid,begin,finish,tap,disabled}:Props){
 ...
 return <div className="beat-tap">
  ...
  {grid&&grid.count<count&&<p className="config-hint" role="status">最新のタップでは推定が安定しなかったため、{grid.count}回目までの有効な結果を保持しています。</p>}<details className="beat-tap-help"><summary>使い方・補正の内訳</summary><p className="config-hint">動画を再生し、1、2、3…の毎拍でタップ。最初のタップを「1」として、すべての拍の位置とBPMを一緒に平滑化します。</p>{grid&&<p className="config-hint">タップのばらつき：{Math.round(grid.errorMs)}ms。上の点は実際のタップ、白線と下の点は等間隔に補正した拍。オレンジは計算から除外したタップです。</p>}<p className="config-hint">64回は不要です。2回以上の有効な結果はその都度採用・保存します。4〜16回ほどあると安定しやすくなります。全タップが一律に遅れた場合は「拍の位置」やトラックで調整してください。</p></details>
 </div>;
}
