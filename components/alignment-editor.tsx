 ...
 return <section className="alignment-editor" aria-label="重ね合わせ調整">
  ...
  <Tabs value={tab} onValueChange={v=>setTab(String(v))}>
   ...
   <TabsContent value="camera" className="alignment-editor-body">
    <p>同じ姿勢で止め、自分の映像の上下・左右の大きさを調整。補正は再生中も固定します。</p>
   ...
   <TabsContent value="position" className="alignment-editor-body alignment-controls">
   ...
   <TabsContent value="form" className="alignment-editor-body">{form}<p>遠近の数値は撮影角度ではなく、見た目を近づける強さです。隠れた手足は復元できません。</p><p>表示は補正前の2D角度差です。撮影角度の影響も含むため、フォームの正誤の判定には使いません。</p></TabsContent>
  </Tabs>
  <footer><button className="button primary wide" disabled={busy||disabled} onClick={auto}>{busy?'骨格を読み取り中…':'この遠近で骨格の位置・倍率を合わせる'}</button></footer>
 </section>;
}
