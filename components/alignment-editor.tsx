...
type Props={manualOnly?:boolean;background:ReactNode;alignment:Alignment;change:(key:keyof Alignment,value:number)=>void;auto:()=>void;autoSequence:()=>void;sequenceHint:string;busy:boolean;disabled:boolean;close:()=>void;reset:()=>void;form:ReactNode};
export function AlignmentEditor({manualOnly=false,background,alignment:a,change,auto,autoSequence,sequenceHint,busy,disabled,close,reset,form}:Props){
 ...
 return <section className={'alignment-editor'+(manualOnly?' manual-alignment':'')} aria-label="重ね合わせ調整">
  ...
  <Tabs value={tab} onValueChange={v=>setTab(String(v))}>
   ...
   <TabsContent value="camera" className="alignment-editor-body">
    <p>{manualOnly?'自分の映像の上下・左右の大きさを手動で調整します。':'壁・窓を基準にする画角補正は、各動画の「設定 → 画角」で試せます。ここでは残った遠近差を手動で調整します。'}</p>
   ...
   <TabsContent value="position" className="alignment-editor-body alignment-controls"><p>{manualOnly?'自分の濃さ・倍率・位置を手動で合わせます。上の「YouTubeを操作」でお手本だけを表示できます。':sequenceHint||'「動画全体で自動補正」で対象の人を選ぶと、複数の場面から倍率と位置を合わせます。傾き・遠近は維持します。'}</p>
