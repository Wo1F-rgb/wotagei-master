...
export function BackgroundControls({settings:s,change,disabled}:{settings:BackgroundSettings;change:(settings:BackgroundSettings)=>void;disabled:boolean}){
 ...
 <p className="config-hint">{disabled?'YouTubeの埋め込み動画は切り抜けません。動画ファイルを選ぶと使えます。':s.mode==='green'?'緑の服・サイリウムも消える場合があります。重ねる画面では背景は透明になります。':s.mode==='person'?'緑背景で輪郭を確認できます。「重ねる」では透明に。速い動きやサイリウムは欠ける場合があります。':'動画とインカメの背景を消して、人物を見やすくできます。'}</p>
