...
type Props=Omit<TrackControls,'disabled'>&{controlsDisabled:boolean;durations:number[];origins:number[];bpm:number[];known:boolean[];time:number;playing:boolean;camera:boolean;disabled:boolean;seek:(t:number)=>void;adjust:(i:number,t:number)=>void;pause:()=>void;save:()=>void};
export function VideoTracks({master,rate,changeMaster,changeRate,canSelf,controlsDisabled,nudgeDisabled,nudgeStep,nudge,scope,durations,origins,bpm,known,time,playing,camera,disabled,seek,adjust,pause,save}:Props){
 ...
 return <div className="video-tracks">
  ...
  {[0,1].map(i=>{const bounds=trackBounds(durations[i],origins[i],effective[i],refBpm,0),enabled=durations[i]>0&&!(i===1&&camera)&&!disabled;return <div className={`track-row track-${i}`} key={i}>
   <span className="track-name" data-selected={selected===i}>{i===0?'お手本':'自分'}<small>{i===1&&camera?'LIVE':timeLabel(origins[i])}</small></span>
   <div className="track-ruler">
    ...
    {enabled?<button className="track-clip" disabled={i===master} style={{left:`${(bounds.start-start)/span*100}%`,width:`${Math.max(.2,(bounds.end-bounds.start)/span*100)}%`}} aria-label={i===master?`${i===0?'お手本':'自分'}は主役のため固定`:`${i===0?'お手本':'自分'}のトラックを左右にずらす`} role="slider" aria-valuemin={0} aria-valuemax={durations[i]} aria-valuenow={origins[i]} aria-valuetext={`最初の1は${origins[i].toFixed(4)}秒`} onPointerDown={e=>{if(e.button!==0||i===master)return;pause();const width=e.currentTarget.parentElement!.getBoundingClientRect().width;dragging.current={i,x:e.clientX,origin:origins[i],pps:width/span};e.currentTarget.setPointerCapture(e.pointerId);}} onPointerMove={e=>{const d=dragging.current;if(d?.i===i)adjust(i,timelineShift(d.origin,e.clientX-d.x,d.pps,effective[i],refBpm,durations[i]));}} onPointerUp={e=>{dragging.current=null;e.currentTarget.releasePointerCapture(e.pointerId);save();}} onPointerCancel={()=>{dragging.current=null;save();}} onKeyDown={e=>{if(i!==master&&(e.key==='ArrowLeft'||e.key==='ArrowRight')){e.preventDefault();nudge(e.key==='ArrowLeft'?-1:1);}}}><span>{i===0?'お手本':'自分'} {known[i]?`${bpm[i]} BPM`:'BPM未設定'}</span></button>:<span className="track-empty">{i===1&&camera?'インカメ · ライブ':'動画を選ぶ'}</span>}
    <span className="track-one" style={{left:`${-start/span*100}%`}}>1</span><span className="track-cursor" style={{left:`${(cursor-start)/span*100}%`}}/>
   </div>
  </div>;})}
  <div className="track-seek"><span>{timeLabel(time)}</span><Slider aria-label={camera?"お手本の再生位置":"2つの動画の再生位置"} value={[time]} min={0} max={Math.max(.001,durations[0])} step={.01} disabled={!durations[0]||disabled} onValueChange={v=>{const t=Array.isArray(v)?v[0]:v;seek(t);setStart(t-origins[0]-span*.3);}}/><span>{timeLabel(durations[0])}</span></div>
 </div>;
}
