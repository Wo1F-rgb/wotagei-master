...
type Props={remote?:boolean;file:File|null;live:boolean;bpm:number;kind:BpmKind;tapCount:number;tap:()=>void;resetTaps:()=>void;apply:(value:number,kind:BpmKind)=>void;pause:()=>void};
export function TempoConfig({remote=false,file,live,bpm,kind,tapCount,tap,resetTaps,apply,pause}:Props){
 const [mode,setMode]=useState(remote?'songs':'auto'),[text,setText]=useState(kind==='unset'?'':String(bpm));
 ...
 function adopt(value:number,from:BpmKind){apply(value,from);setMessage(`${value} BPMを設定しました。「①『1』」で開始位置も合わせてください。`);}
 ...
 return <div className="tempo-config">
  ...
  <Tabs value={mode} onValueChange={v=>{setMode(String(v));setMessage('');}} className="tempo-methods">
   <TabsList aria-label="BPMを決める方法"><TabsTrigger value="auto">自動解析</TabsTrigger><TabsTrigger value="manual">入力・TAP</TabsTrigger><TabsTrigger value="songs">曲から選ぶ</TabsTrigger></TabsList>
   ...
   <TabsContent value="manual" className="tempo-body">
    <div className="manual-tempo"><label className="numeric"><span>この動画のBPM</span><input type="number" inputMode="decimal" min={40} max={300} step={.1} placeholder="例：150" value={text} onChange={e=>setText(e.target.value)} aria-label="この動画のBPM"/></label><button className="button" disabled={!valid||(!file&&!remote)} onClick={()=>adopt(Number(text),'manual')}>設定</button><button className="tap-target" disabled={!file&&!remote} onPointerDown={e=>{if(e.button===0){e.preventDefault();tap();}}} onClick={e=>{if(e.detail===0)tap();}}>TAP<span>{tapCount<6?`${tapCount}/6回`:`${tapCount}回で推定`}</span></button><button className="button mini" onClick={resetTaps} aria-label="TAPをやり直す">↺</button></div>
    <p className="config-hint">等速再生して毎拍タップ。16〜24回で安定しやすくなります。8拍に1回ではありません。</p>
   </TabsContent>
