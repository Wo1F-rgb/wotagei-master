...
type Props={options:SequenceOptions;close:()=>void;apply:(alignment:PoseAlignment,frames:number)=>void;applyMotion?:(data:MotionAnalysis)=>void};
...
export function AutoAlignment({options,close,apply,applyMotion}:Props){
 ...
 async function run(){
  ...
  try{
   ...
   else{const result=await current.analyze(selection,progress);if(!signal.aborted)callbacks.current.apply(result.alignment,result.frames);}
  }
 ...
 return <Dialog open onOpenChange={open=>{if(!open)cancel();}}><DialogContent className="practice-dialog auto-alignment-dialog"><DialogTitle>{options.motion?'手持ち動画の追従を解析':'動画全体で自動サイズ補正'}</DialogTitle><DialogDescription>{running?'同じ人を確認できる場面から、倍率と位置を計算しています。':'番号をタップして、2本それぞれの対象人物を確認してください。'}</DialogDescription>
  <div className="auto-alignment-body">
   ...
   {!running&&!error&&<p className="config-hint">{options.motion?'先にBPM・拍と基本の位置を合わせて使います。同じ拍の場面を細かく解析し、三脚OFF側の左右・上下・倍率の変化を滑らかに補正します。長い動画は解析に時間がかかります。':'BPMと拍の位置を先に合わせて使います。仁王立ちは不要です。動画全体の32場面を端末内で解析し、傾き・遠近は現在の設定を維持します。'}</p>}
  </div>
