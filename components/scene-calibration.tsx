...
export function SceneCalibrationEditor({video,saved,preview:showPreview,save}:Props){
 ...
 <p className="config-hint">{draft.mode==='level'?'背景の水平な線の左端①・右端②を選びます。足先や斜め奥へ続く床の線は基準にしません。':'壁・窓・鏡など、実物が長方形の四隅を①左上→②右上→③右下→④左下の順に選びます。人体や床面の四角は使いません。'}</p>
 {draft.mode==='rectangle'&&<label className="scene-ratio">実物の横÷縦<input aria-label="基準の長方形の実物の横幅を高さで割った値" type="number" inputMode="decimal" min=".2" max="5" step=".01" value={ratio} placeholder="例 100÷200 → 0.5" onChange={e=>{setRatio(e.target.value);setPreviewing(false);}}/><button className="button mini" onClick={()=>{setRatio('1');setPreviewing(false);}}>正方形</button></label>}
